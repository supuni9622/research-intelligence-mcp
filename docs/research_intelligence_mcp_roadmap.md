...
- [x] Local observability stack (`docker-compose.yml` +
      `deployment/observability/`): Prometheus scraping `/metrics`, Grafana
      pre-provisioned with a 10-panel dashboard reading from it — verified
      end to end (target `up`, panel queries return live data generated by
      `smoke_test.py`, dashboard/datasource survive a Grafana restart). Dev
      only; not a production observability stack — see the deployment
      guide §8/§9.
