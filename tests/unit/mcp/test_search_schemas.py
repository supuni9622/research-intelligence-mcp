...
def test_builds_default_federated_search_input() -> None:
    ...
    search_input = SearchPapersInput(query="retrieval augmented generation")

    assert search_input.query == ("retrieval augmented generation")
...
def test_normalizes_query_whitespace() -> None:
    ...
    search_input = SearchPapersInput(query="  retrieval   augmented\n generation  ")
...
def test_converts_to_canonical_search_request() -> None:
    ...
    assert request.providers == (ProviderName.ARXIV,)
    assert request.limit == 5
    assert request.offset == 10
    assert request.year_from == 2022
    assert request.year_to == 2026
    assert request.fields_of_study == ("cs.IR",)
    assert request.open_access_only is True
    assert request.sort == (SearchSort.PUBLICATION_DATE)
...
def test_rejects_invalid_year_range() -> None:
    ...
    with pytest.raises(
        ValidationError,
        match=("year_from must be less than or equal to year_to"),
    ):
        SearchPapersInput(
            query="language models",
            year_from=2026,
