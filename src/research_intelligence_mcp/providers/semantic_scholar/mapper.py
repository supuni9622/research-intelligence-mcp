...
class SemanticScholarMapper:
    ...
    def to_author(
        ...
        return Author(
            name=name,
            semantic_scholar_id=cls._clean_optional_string(response.author_id),
            affiliations=(),
            homepage_url=None,
        )
    ...
    def to_identifiers(
        ...
        external_ids = response.external_ids or SemanticScholarExternalIdsResponse()
        ...
        corpus_id = cls._normalize_corpus_id(raw_corpus_id)

        semantic_scholar_id = cls._normalize_semantic_scholar_id(response.paper_id)

        return PaperIdentifiers(
            doi=cls._clean_optional_string(external_ids.doi),
            arxiv_id=cls._clean_optional_string(external_ids.arxiv),
            semantic_scholar_id=semantic_scholar_id,
            corpus_id=corpus_id,
            pmid=cls._clean_optional_string(external_ids.pubmed),
        )
    ...
    def to_access(
        ...
        pdf_url_value = (
            cls._clean_optional_string(open_access_pdf.url)
            if open_access_pdf is not None
            else None
        )

        landing_page_url_value = cls._clean_optional_string(response.url)

        license_name = (
            cls._clean_optional_string(open_access_pdf.license)
            if open_access_pdf is not None
            else None
        )
        ...
        return PaperAccess(
            status=status,
            landing_page_url=cls._parse_http_url(landing_page_url_value),
            pdf_url=cls._parse_http_url(pdf_url_value),
            license=license_name,
            repository=(
                ProviderName.SEMANTIC_SCHOLAR if pdf_url_value is not None else None
            ),
        )
    ...
    def to_paper(
        ...
        title = cls._clean_optional_string(response.title)
        ...
        authors = tuple(
            ...
            if (author := cls.to_author(item)) is not None
        )
        ...
        return Paper(
            ...
            title=title,
            authors=authors,
            abstract=cls._clean_optional_string(response.abstract),
    ...
    def citation_to_reference(
        ...
        return PaperReference(
            ...
            paper=cls.to_paper(response.citing_paper),
            contexts=tuple(cls._clean_string_collection(response.contexts)),
            intents=tuple(cls._clean_string_collection(response.intents)),
    ...
    def reference_to_reference(
        ...
        return PaperReference(
            ...
            paper=cls.to_paper(response.cited_paper),
            contexts=tuple(cls._clean_string_collection(response.contexts)),
            intents=tuple(cls._clean_string_collection(response.intents)),
            is_influential=response.is_influential,
        )
    ...
    def to_citations(
        ...
        return [
            ...
            if (reference := cls.citation_to_reference(edge)) is not None
        ]
    ...
    def to_references(
        ...
        return [
            reference
            for edge in response.data
            if (reference := cls.reference_to_reference(edge)) is not None
        ]
    ...
    def to_search_result(
        ...
        *,
        ...
        papers = [cls.to_paper(item) for item in response.data]

        warnings: list[str] = []

        if request.fields_of_study:
            warnings.append(
                "Semantic Scholar field-of-study filtering was applied locally."
            )

            requested_fields = {field.casefold() for field in request.fields_of_study}

            papers = [
                paper
                for paper in papers
                if requested_fields.intersection(
                    field.casefold() for field in paper.fields_of_study
                )
            ]
        ...
        pagination = PaginationMetadata(
            ...
            has_more=(response.next_offset is not None),
        )
        ...
        return SearchResult(
            ...
            providers_succeeded=(ProviderName.SEMANTIC_SCHOLAR,),
            failures=(),
    ...
    def _normalize_semantic_scholar_id(
        ...
        normalized = SemanticScholarMapper._clean_optional_string(value)
    ...
    def _resolve_access_status(
        *,
        ...
        open_access_pdf: (SemanticScholarOpenAccessPdfResponse | None),
    ) -> AccessStatus:
        """Resolve canonical paper access status."""

        has_pdf = open_access_pdf is not None and bool(
            SemanticScholarMapper._clean_optional_string(open_access_pdf.url)
        )
    ...
    def _resolve_venue(
        ...
        venue = cls._clean_optional_string(response.venue)

        if venue is not None:
            return venue

        if response.publication_venue is not None:
            publication_venue_name = cls._clean_optional_string(
        ...
        if response.journal is not None:
            return cls._clean_optional_string(response.journal.name)
    ...
    def _resolve_fields_of_study(
        ...
        fields = list(response.fields_of_study)
        ...
        return cls._clean_string_collection(fields)
    ...
    def _clean_string_collection(
        ...
        for value in values:
            cleaned = cls._clean_optional_string(value)
